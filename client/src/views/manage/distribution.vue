<template>
  <div class="g-content-box">
    <div class="g-page-title">质控分配</div>
    <div class="g-form">
      <el-form :inline="true" :model="formInline" ref="formInline" class="demo-form-inline">
        ...
        <el-form-item prop="allState">
          <el-select v-model="formInline.allState" placeholder="全部分配状态">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span v-if="scope.row.pfState=='0'" class="g-no-color">未评分</span>
            <span v-else class="g-yes-color">已评分</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="state"
          ...
          align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.state=='0'" class="g-no-color">未分配</span>
            <span v-else class="g-yes-color">已分配</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="state"
          ...
          <el-button
            class="g-success"
            @click.native.prevent="distribution(scope.$index, tableData)"
            type="success"
          ...
          <el-button
            class="g-delete"
            @click.native.prevent="cancelDistribution(scope.$index, tableData)"
            type="danger"
            v-else
            size="mini">
            取消分配
...
export default {
  ...
  methods: {
    ...
    cancelDistribution (index, rows) {
      this.$confirm('取消分配, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
        center: true
